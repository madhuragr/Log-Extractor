...
def binary_search_row(f, start, end, i_pointer, start_timestamp, end_timestamp):
	...
	if (start_timestamp == timestamp):
		...
		# print ("middle")
		f.seek(num_chars *(i - 1))
		t1 = threading.Thread(target=print_rows_below, args=(f, i, num_chars, i_pointer, end_timestamp)) 
		t1.start()
		# threading.current_thread().name = "my_name"
		i_pointer = i # this should be updated only after t1 thread is finished
		t2 = threading.Thread(target=binary_search_row, args=(f, start, i-1, i_pointer, start_timestamp, end_timestamp)) 
		# THIS NEEDS TO BE TESTED PERFECTLY. 
		# i IN PRINT FUNC SHOULD HAVE MOVE PAST THE CONDITION BY THE TIME ABOVE ASSIGNMENT IS DONE.
		t2.start()
		t1.join()
		t2.join()
...
binary_search(1, 10, start_timestamp, end_timestamp)
end = time.time()
