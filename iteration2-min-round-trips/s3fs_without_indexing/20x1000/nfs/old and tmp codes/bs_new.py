...
def print_rows_below(f, end_timestamp):
	for i,line in enumerate(f):
		if (i == i_pointer - 1):
			return # i starts from 0, line no starts from 1 
		date_time = line[0:26]
...
def print_below_files(i, end_timestamp):
	...
	with open('LogFile-%s.log' % i_padded, 'rb') as f:
		for j,line in enumerate(f):
				...
				if (timestamp <= end_timestamp):
					...
					return
	f.close()
	print_below_files(i+1, end_timestamp)
...
def binary_search_row(f, start, end, start_timestamp, end_timestamp):
	# f.seek(0)
	if(start > end):
		return # check
	...
	if (start_timestamp == timestamp):
		...
		f.seek(num_chars *(i - 1))
		print_rows_below(f, end_timestamp) 
	
	elif (start_timestamp > timestamp):
		binary_search_row(f, i+1, end, start_timestamp, end_timestamp)
	
	elif (end_timestamp < timestamp):
		binary_search_row(f, start, i-1, start_timestamp, end_timestamp)
	
	elif (start_timestamp < timestamp and timestamp <= end_timestamp):
		# file pointer has moved ahead by one line 
		# i_pointer : lines below this line no have already been printed
		f.seek(num_chars *(i - 1))
		t1 = threading.Thread(target=print_rows_below, args=(f, end_timestamp)) 
		t2 = threading.Thread(target=binary_search_row, args=(f, start, i-1, start_timestamp, end_timestamp)) 
		t1.start()
		i_pointer = i # this should be updated only after t1 thread is finished .. need to test this
		t2.start()
...
def binary_search(start, end, start_timestamp, end_timestamp):
	# if(start == end):
	# 	# return start
	# 	# Means same file for start and end query. So, binary search similar function for rows
	...
	# 	binary_search_row(f_start, 1, no_of_lines, start_timestamp, end_timestamp)
	...
	if (end - start == 1):
		...
		if(end_timestamp <= last_timestamp_start_file):
			...
			binary_search_row(f_end, 1, no_of_lines, start_timestamp, end_timestamp)
		...
		with open('LogFile-%s.log' % i_padded, 'rb') as f:
			...
			if (start_timestamp == timestamp):
				...
				t2 = threading.Thread(target=print_below_files, args=(i+1, end_timestamp)) 
				t1.start()
				t2.start()
...
binary_search(1, 200, start_timestamp, end_timestamp)
